...
object ActionHandler {
    ...
    /** Returns a spoken confirmation message. */
    suspend fun handle(activity: Activity, action: JSONObject, prefs: Prefs): String {
        return when (action.optString("action")) {
            "alarm", "reminder" -> setAlarm(activity, action)
            "timer" -> setTimer(activity, action)
            "call" -> makeCall(activity, action)
            "sms" -> sendSms(activity, action)
            "sos" -> sos(activity, prefs)
            "weather" -> WeatherClient.forCity(prefs.city)
            "calendar" -> addCalendar(activity, action)
            "search" -> search(activity, action)
            else -> action.optString("text", "Done.")
        }
    }

    private fun setAlarm(activity: Activity, a: JSONObject): String {
    ...
    private fun setTimer(activity: Activity, a: JSONObject): String {
    ...
    private fun makeCall(activity: Activity, a: JSONObject): String {
        val number = resolveNumber(activity, a) ?: return "I couldn't find that contact."
    ...
    private fun sendSms(activity: Activity, a: JSONObject): String {
        val number = resolveNumber(activity, a) ?: return "I couldn't find that contact."
        val message = a.optString("message", "")
        if (message.isBlank()) return "What should the message say?"
        return try {
            smsManager(activity).sendTextMessage(number, null, message, null, null)
    ...
    private fun sos(activity: Activity, prefs: Prefs): String {
        ...
        val loc = LocationHelper.lastLocationLink(activity)
        val body = "SOS! I need help. My location: ${loc ?: "location unavailable"}"
        return try {
            smsManager(activity).sendTextMessage(number, null, body, null, null)
    ...
    private fun addCalendar(activity: Activity, a: JSONObject): String {
        ...
        if (a.has("start_millis")) {
            i.putExtra(CalendarContract.EXTRA_EVENT_BEGIN_TIME, a.optLong("start_millis"))
        }
        return try {
            activity.startActivity(i)
    ...
    private fun search(activity: Activity, a: JSONObject): String {
    ...
    private fun smsManager(activity: Activity): SmsManager {
    ...
    private fun resolveNumber(activity: Activity, a: JSONObject): String? {
        val direct = a.optString("number", "")
        if (direct.isNotBlank()) return direct
        val contact = a.optString("contact", "")
        if (contact.isBlank()) return null
        return lookupContact(activity, contact)
    }

    private fun lookupContact(activity: Activity, name: String): String? {
        ...
        activity.contentResolver.query(uri, proj, sel, args, null)?.use { c ->
